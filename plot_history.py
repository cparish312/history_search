...
def get_clusters(time_bin="H"):
    ...
    url_to_cluster = {u : c for u, c in zip(urls, kmeansModel.labels_)}
    results_history = history.loc[history['url'].isin(urls)]
    results_history['cluster'] = results_history['url'].map(url_to_cluster)
